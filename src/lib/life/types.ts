...
export type HabitDay = {
  intent?: "develop" | "reduce";
  value: number;
  target: number;
  at?: string;
  confirmed: boolean;
};
export type HabitTracking = LifeFields & {
  intent?: "develop" | "reduce";
  quantityTarget?: number;
  unit?: string;
  records?: Record<string, HabitDay>;
  alternative?: string;
  alternativeHabitId?: string;
  limit?: number;
};
...
export type LifeEvent = LifeFields & {
  id: string;
  name: string;
  category: string;
  date: string;
  ...
  endDate?: string;
  kind: "point" | "interval";
  source: "manual" | "habit" | "task" | "focus";
  sourceId?: string;
  habitId?: string;
  goalId?: string;
  context?: string;
...
export type DirectionVersion = LifeFields & {
  at: string;
  name: string;
  goalId?: string;
  archived?: boolean;
...
export type LifeData = {
  ...
  areaSpheres: Record<string, Sphere>;
  indicators: Indicator[];
  events: LifeEvent[];
  marks: StateMark[];
  days: Record<string, DailyLog>;
  archivedHabits: import("../../data/seed").Habit[];
  directions: Direction[];
  choices: Record<string, ChoiceSetting>;
  choiceShownDate?: string;
