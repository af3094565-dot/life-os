...
export function HabitFormModal({
  ...
  const steps: WizardStep[] = useMemo(() => {
    const list: WizardStep[] = [
      {
        ...
        title: 'Название',
        hint: area
          ? `Привычка для аспекта «${area.title}»`
          : 'Как будешь называть эту привычку?',
        content: (
    ...
    if (!isEdit && goals.length > 0 && !lockGoal) {
      list.push({
        id: 'goal',
        title: 'Цель',
    ...
    } else if (!isEdit && lockGoal) {
      const g = goals.find((x) => x.id === defaultGoalId) ?? goals.find((x) => x.id === goalId)
      list.push({
        id: 'goal-locked',
        title: 'Цель',
        hint: area
          ? 'Закрепится за аспектом карты жизни'
        ...
        content: (
          <div className="rounded-xl bg-emerald-50 px-3 py-3 text-sm font-semibold text-emerald-950 ring-1 ring-emerald-100">
            {g ? `Закреплено: ${g.title}` : area ? `Аспект: ${area.short}` : 'Закреплено'}
    ...
    list.push(
      {
        id: 'start',
        title: 'Дата старта',
        hint: 'Можно запланировать на потом',
        content: (
          <div className="space-y-2">
            <input
              type="date"
            ...
            />
            {highlight.start && (
              <p className="text-sm font-semibold text-danger">Укажи дату старта</p>
            )}
            <input
              type="time"
              value={reminderTime}
          ...
          <div className="space-y-3">
            <div className="rounded-xl bg-canvas px-3 py-3 ring-1 ring-line">
              ...
              <p className="mt-1 text-sm font-medium text-muted">
                с {startLabel} до ~{endLabel} · {targetDays}× · {timesPerWeek}/нед
              </p>
              {area && (
                ...
                <div className="rounded-xl bg-sky-50 px-3 py-3 text-sm text-sky-950 ring-1 ring-sky-100">
                  <p className="font-semibold">
                    {cost <= 0
                      ...
                      : `−${formatDiamonds(cost)} · у тебя ${formatDiamonds(diamonds)}`}
                  </p>
